...
for i in range(100000):
    if i==0:
        ...
        x[0,i] = v[0,i] + 0.5*v[0,i-1]*v[0,i-2]
...
for t in range(2500):
    for ctr in range(1000):
        i = random.randint(6,99999)
        x_1 = np.array([[v[0,i],v[0,i-1],v[0,i-2],v[0,i-3],v[0,i-4],v[0,i-5]]])              # Input in layer 1 (1X6)
    ...
    num = 1000
    set = np.zeros((1,num))
    v_new = np.random.normal(loc = 0,scale = 1,size=(1,num))
    x_new = np.zeros((1,num))
    for i in range(num):
        if i==0:
            x_new[0,i] = v_new[0,0]
        elif i==1:
            ...
            x_new[0,i] = v_new[0,i] + 0.5*v_new[0,i-1]*v_new[0,i-2]
    for k in range(0,6):
        set[0,k] = v_new[0,k]
    for k in range(6,num):
        x_1 = np.array([[v_new[0,k],v_new[0,k-1],v_new[0,k-2],v_new[0,k-3],v_new[0,k-4],v_new[0,k-5]]]) 
